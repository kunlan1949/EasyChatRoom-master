...
//        close.setOnClickListener(listener);
        ...
        userRoomName=findViewById(R.id.roomName);
    ...
    public void onClick(View v) {
        ...
        String nana=userRoomName.getText().toString();
        if (nana==null) {
            roomName="未命名";
        }else {
            roomName = userRoomName.getText().toString();
        }
        if (userName.equals("")) {
...
//                    Log.d(TAG, "onClick: 服务是否运行：" + CostomUtil.isServiceRunning(this,ServerService.CLASSNAME));
                    startServer();
                    joinChat(roomName + " 的聊天室",CostomUtil.getIPAddress(this));
